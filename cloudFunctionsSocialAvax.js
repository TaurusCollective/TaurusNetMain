...
Moralis.Cloud.afterSave("CommentsAVAX", async (request) => {
  ...
  if(confirmed){
    ...
    const query = new Moralis.Query("PostsAVAX");
...
Moralis.Cloud.afterSave("PostsAVAX", async (request) => {     
  const confirmed = request.object.get("confirmed");
  if(confirmed){
    const logger = Moralis.Cloud.getLogger();
    ...
    await Moralis.Cloud.run("AddPostsAVAXToMultichain", params);
...
Moralis.Cloud.afterSave("UserOnChainAVAX", async (request) => {     
  const confirmed = request.object.get("confirmed");
  if(confirmed){
    const logger = Moralis.Cloud.getLogger();
