...
import Comments from "./CommentsAvax";
import CommentsAvax from "./CommentsAvax";
import VotesAvax from "./VotesAvax";
...
const PostAvax = ({post}) => {
    ...
    const { data } = useMoralisQuery("ContentsAVAX", (query) => query.equalTo("contentId", contentId));
    //const [voteStatus, setVoteStatus] = useState();
    const { data: votes } = useMoralisQuery("VotesAVAX", (query) => query.equalTo("postId", postId), [], {
        live: true,
    });
      ...
      <div className="bg-white my-7 border rounded-sm">
            ...
            <div className="flex item-center paddingHeader items-center">
                ...
                <p className="flex-1 font-bold">@{postOwnerUsername}</p>
                  ...
                  <img src="/images/AvaxLogo.png" alt="" className="w-5 h-5"/>
            </div>
            ...
            {commentList && ( 
              ...
              {commentList.map((commentLink, index) =>{
                // return   <Comments key={commentLink} commentLink={commentLink}/>
                 return   <CommentsAvax key={index} commentLink={commentLink}/>
              })}
              </div>
            )}
              {/* {commentList && ( 
